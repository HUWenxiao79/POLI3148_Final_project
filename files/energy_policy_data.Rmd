...
## separate different countries

```{r}
d_policy_grouped <- d_policy %>%
  group_by(Country)
```

#create dataframes for each country and print them

```{r}
for(name in unique(d_policy$Country)){
  d_policy_group <-d_policy_grouped %>% filter(Country == name)
  filename <-file.path("data",paste0(name,'.csv'))
  write_csv(d_policy_group,filename)
}
```
...
```{r}
d_Argentina <- read.csv("data/policy_by_countries_cleaned/Argentina.csv")
d_Argentina
```
